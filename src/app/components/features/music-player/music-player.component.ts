...
export class MusicPlayerComponent {
  ...
  $currentFormattedTime = signal<string>('00:00');
  ...
  private handleTimeUpdate(): void {
    ...
    const aCurrentTime = this.audio.currentTime;
    const aDuration = this.audio.duration;
    const percent = (aCurrentTime / aDuration) * 100;
    animate(el, { flexBasis: `${percent}%` });
    this.$currentTime.set(aCurrentTime);
    this.$currentFormattedTime.set(this.formatTime(aCurrentTime));
  }
